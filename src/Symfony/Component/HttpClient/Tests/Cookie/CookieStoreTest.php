...
class CookieStoreTest extends TestCase
{
    ...
    public function testExtractFromResponseSkipsNonRfcCompliantCookies(): void
    {
        $response = $this->createMock(ResponseInterface::class);
        $response->expects($this->once())->method('getHeaders')->with(true)->willReturn([
            'set-cookie' => [
        ...
        $this->assertCount(2, $store);
        $this->assertSame('ok', $store->getCookie('valid')->getValue());
        $this->assertSame('fine', $store->getCookie('also_valid')->getValue());
        $this->assertNull($store->getCookie('bad name'));
    }
