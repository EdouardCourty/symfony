 ...
 */
final class CookieStore implements \Stringable, \Countable, \IteratorAggregate
{
    ...
    public static function extractFromResponse(ResponseInterface $response, bool $throw = true): self
    {
        ...
        foreach ($response->getHeaders($throw)['set-cookie'] ?? [] as $header) {
            ...
            // We extract only the first "name=value" segment.
            ...
            try {
                $cookie = Cookie::fromString(explode(';', $header, 2)[0]);
     ...
     */
    public function withCookie(string|Cookie $name, string $value = ''): static
    {
        $cookie = $name instanceof Cookie ? $name : new Cookie($name, $value);
    ...
    /**
     * Returns an iterator over the Cookie objects, keyed by cookie name.
     *
