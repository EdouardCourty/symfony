    ...
    private static function prepareRequest(?string $method, ?string $url, array $options, array $defaultOptions = [], bool $allowExtraOptions = false): array
    {
        ...
        if (null !== $url) {
            ...
            if (isset($options['cookies'])) {
                ...
                } elseif (!$cookies instanceof CookieStore) {
                    throw new InvalidArgumentException(\sprintf('Option "cookies" must be a string, an array, or a "%s", "%s" given.', CookieStore::class, get_debug_type($cookies)));
                }
                if ('' !== (string) $cookies) {
